...
if [[ $(grep After /etc/openvpn/easy-rsa/keys/$item | awk '{print $7}') = $(date +%Y) && $(grep After /etc/openvpn/easy-rsa/keys/$item | awk '{print $4}') = $(date +%b) && $(grep After /etc/openvpn/easy-rsa/k$echo $item >> /tmp/keys_expired.txt
